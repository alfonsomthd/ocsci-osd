...
PROV_CLUSTER_DIR="${PROV_CLUSTER_DIR:=/opt/cluster-prov}"
...
echo "- PROV_CLUSTER_DIR: $PROV_CLUSTER_DIR"
...
echo -n "- " && mkdir -pv "${PROV_CLUSTER_DIR}"/{auth,logs}
...
export CEPHADMINKEY=$(oc get secret ci-secrets -n osde2e-ci-secrets -o jsonpath='{.data.provider-ceph-admin-key}' | base64 -d)

echo "### Running run-ci."
cd "$OCSCI_INSTALL_DIR"
source venv/bin/activate

run-ci multicluster 2 "${OCSCI_INSTALL_DIR}/tests/manage/pv_services/test_pvc_assign_pod_node.py" -m acceptance \
    --junit-xml="${JUNIT_XML}" \
    --ocsci-conf="${OCSCI_INSTALL_DIR}/conf/ocsci/disable_pagerduty.yaml" \
    --cluster1 --cluster-name e2e-cons-addon \
    --cluster-path "${CLUSTER_DIR}" --ocp-version 4.10 --ocs-version 4.10 \
    --ocsci-conf="${OCSCI_INSTALL_DIR}/conf/deployment/rosa/managed_3az_consumer_qe_3m_3w_m52x.yaml" \
    --ocsci-conf="${OCSCI_INSTALL_DIR}/conf/deployment/rosa/consumer_cluster_type.yaml" \
    --cluster2 --cluster-name e2e-prov-addon \
    --cluster-path "${PROV_CLUSTER_DIR}" --ocp-version 4.10 --ocs-version 4.10 \
    --ocsci-conf="${OCSCI_INSTALL_DIR}/conf/deployment/rosa/managed_3az_provider_qe_3m_3w_m54x.yaml" \
    --ocsci-conf="${OCSCI_INSTALL_DIR}/conf/deployment/rosa/provider_cluster_type.yaml"
